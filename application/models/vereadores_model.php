...
class vereadores_model extends CI_Model {
    ...
    public function getid($id){
        ...
        return $query->result();
          ...
          public function getvereador($id){
        $query = $this->db->query('select * from funcionarios WHERE id ='.$id);
        return $query->result();
        }
        
        public function getvereadores($id) {
            echo $id;
       $query = $this->db->query('select * from funcionarios WHERE id ='.$id.' AND entidade = '.$this->session->userdata('entidade'));
       echo count($query); 
       return $query->result();
    ...
    public function getall() {
        $query = $this->db->query("select * from funcionarios where tipo = 'vereador' ORDER BY ordem ASC");
        return $query->result();
    }
